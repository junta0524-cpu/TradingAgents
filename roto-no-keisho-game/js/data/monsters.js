...
//   1 東の街道      第一章   アルヴァ1人  Lv1〜3   総HP 28
...
Game.Data.Families = {
  ...
  // 金剛系。硬すぎて何を当てても ほとんど通らない。
  // 倒しきる前に逃げるので、まともに削るより 渾身の一撃を待つほうが早い。
  metal:    { name: '金剛系',   physical: 0.04, fire: 0.04, ice: 0.04, blast: 0.06, wind: 0.04, light: 0.06, ailment: 0.0 },
};
...
var SPELL = {
  igna:   { kind: 'spell',  name: 'イグナ',       power: 1.15, element: 'fire' },
  folga: { kind: 'spell',  name: 'フォルガ',     power: 1.35, element: 'fire' },
  seed:  { kind: 'spell',  name: 'シード',     power: 1.1,  element: 'ice' },
  ignas: { kind: 'breath', name: 'イグナス',   power: 0.85, target: 'all_party', element: 'fire' },
  honoo:  { kind: 'breath', name: '灼熱の吐息', power: 0.8,  target: 'all_party', element: 'fire' },
  fubuki: { kind: 'breath', name: '凍てつく吐息', power: 0.9,  target: 'all_party', element: 'ice' },
  mina:  { kind: 'heal',   name: 'ミナ',     power: 28 },
  minara:{ kind: 'heal',   name: 'ミナラ',   power: 70 },
  nox:{ kind: 'ailment', name: 'ノクス',  ailment: 'sleep' },
  vani:{ kind: 'ailment', name: 'ヴァニ', ailment: 'confuse' },
};
...
add('yogiri_no_fukurou', '夜霧のフクロウ', 3, ['cliff_road'], { spd: 15, family: 'sky', skills: [SPELL.nox], skillRate: 0.3 });
...
// ---- アンデッド系 ----
add('samayou_yoroi', '空鎧(からよろい)', 2, ['azure_tower'], { def: 11, spd: 5, family: 'undead' });
add('kodai_no_bourei', '古代の亡霊', 2, ['azure_tower'], { family: 'undead', skills: [SPELL.mina], skillRate: 0.3 });
add('hone_no_banpei', '骨の番兵', 2, ['azure_tower'], { def: 9, family: 'undead' });
add('norowareta_gakuto', '呪われた学徒の霊', 5, ['academy_altar'], { family: 'undead', skills: [SPELL.seed, SPELL.vani], skillRate: 0.35 });
add('souhaku_no_moja', '蒼白の亡者', 7, ['forbidden_ritual_chamber'], { inflict: { status: 'poison', chance: 0.3 }, family: 'undead' });
add('dokuro_no_eishousha', '髑髏の詠唱者', 7, ['forbidden_ritual_chamber'], { atk: 52, def: 18, family: 'undead', skills: [SPELL.folga, SPELL.ignas], skillRate: 0.45 });

// ---- 悪魔系 ----
add('warau_kage', '嗤う影', 2, ['azure_plain'], { spd: 13, family: 'demon' });
add('genwaku_no_imp', '幻惑のインプ', 2, ['azure_tower'], { inflict: { status: 'confuse', chance: 0.3 }, family: 'demon', skills: [SPELL.vani], skillRate: 0.3 });
add('jujutsushi_modoki', '呪術師もどき', 3, ['cliff_road'], { family: 'demon', skills: [SPELL.igna, SPELL.nox], skillRate: 0.4 });
add('keiyaku_no_akuma_inu', '契約の悪魔犬', 5, ['academy_altar'], { spd: 28, family: 'demon' });
add('chi_no_daikousha', '血の代行者', 5, ['academy_altar'], { atk: 38, family: 'demon', skills: [SPELL.ignas, SPELL.minara], skillRate: 0.4 });
add('nanamonaki_shito', '名もなき使徒', 7, ['forbidden_ritual_chamber'], { family: 'demon' });

// ---- 深海系 ----
add('hakkou_kurage', '発光クラゲ', 6, ['abyss_depth'], { inflict: { status: 'sleep', chance: 0.25 }, family: 'deep', skills: [SPELL.seed], skillRate: 0.3 });
...
add('subo_no_youtai', '巣母の幼体', 6, ['abyss_depth'], { hp: 120, family: 'deep', skills: [SPELL.mina], skillRate: 0.3 });
...
add('madou_ningyou', '魔導人形', 2, ['azure_tower'], { family: 'ancient', skills: [SPELL.igna], skillRate: 0.3 });
add('tenkyugi_no_shugoju', '天球儀の守護獣', 3, ['azure_tower'], { family: 'ancient' });
add('kuzureshi_kenja_no_genei', '崩れし賢者の幻影', 3, ['azure_tower'], { family: 'ancient', skills: [SPELL.folga, SPELL.minara], skillRate: 0.45 });
...
Game.Data.Monsters.galoz = {
  ...
  bossSkills: [
    ...
    { name: '必殺の一打', kind: 'crit', power: 1.5 },
  ],
};
Game.Data.Monsters.magatsuki = {
  ...
  bossSkills: [
    { name: '禁呪の残滓', power: 1.2, target: 'all_party', element: 'rau' },
...
Game.Data.Monsters.genso_no_katsubo = {
  ...
  bossSkills: [
    ...
    { name: '必殺の一打', kind: 'crit', power: 1.6 },

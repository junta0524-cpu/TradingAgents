...
Game.Audio = (function () {
  ...
  var SFX = {
    ...
    // レベルアップ。この手のRPGの「ちゃらららーん」に当たる短い上昇形
    ...
    // 魔物と出くわした合図。この手のRPGの「デデン」に当たる二音
    encounter: function () {
      phrase(['D4', 'D4'], 0, 0.09, 'square', 0.32);
      phrase(['D3', 'D3'], 0, 0.09, 'triangle', 0.3);
    },
    // 宿屋。おやすみ→朝、の短いジングル
    inn: function () {
      phrase(['C5', 'E5', 'G5', 'C6'], 0, 0.14, 'triangle', 0.26);
      phrase(['C4', 'C4', 'E4', 'G4'], 0, 0.14, 'square', 0.16);
    },
    // 買い物の「毎度あり」
    buy: function () { phrase(['G4', 'C5', 'E5'], 0, 0.06, 'square', 0.26); },
    // たびのきろくに きろくする
    save: function () { phrase(['C5', 'G5', 'E5', 'C6'], 0, 0.1, 'triangle', 0.28); },
    // 空振り。「ミス」「効かなかった」のときの気の抜けた音
    miss: function () { phrase(['A3', 'F3'], 0, 0.07, 'square', 0.18); },
    // 逃走成功
    escape: function () { phrase(['C5', 'D5', 'E5', 'G5', 'C6'], 0, 0.045, 'square', 0.24); },
    // 毒が身を削る音
    poison: function () { phrase(['D#4', 'D4'], 0, 0.1, 'triangle', 0.22); },
  };
  ...
  var BGM = {
    ...
    // ダンジョン。短い音形を執拗に繰り返し、半音で軋ませる。
    // 洞窟に入った瞬間に曲が変わる ―― あの落差がこの手のRPGの怖さを作っている
    dungeon: { step: 0.30, lead:
      ['D4','-','D#4','-','D4','-','A3','-','D4','-','F4','-','E4','-','-','-',
       'D4','-','D#4','-','D4','-','A3','-','C4','-','B3','-','A3','-','-','-'],
      bass:
      ['D2','-','-','-','D2','-','-','-','A2','-','-','-','A2','-','-','-',
       'D2','-','-','-','D2','-','-','-','A#2','-','-','-','A2','-','-','-'] },
    // 街。落ち着いた三拍子

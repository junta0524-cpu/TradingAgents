...
3. 輪郭は、その部位の色をぐっと暗くした色の1ドット線。純粋な黒(#000000)は使わない。
   腕と胴、髪と顔のように、隣り合う部位の境目にも入れる。
4. 人物は2頭身。頭が全身のおよそ45%。首は描かない。手足は太さ2〜3ドットで、指は描かない。
5. 顔は、目の黒い点がふたつだけ。白目・瞳孔・まつげ・眉・鼻は描かない。口は描いても1ドット。
   目は顔の下半分に置く(上に寄せると大人びて、この絵柄から外れます)。
6. この大きさでつぶれるものは最初から描かない ―― 爪、歯、耳の形、髪の一本一本、
   鎧の留め金、布の皺、刺繍、瞳のハイライト、頬の赤み、傷跡。
   残すのはシルエット・髪の色・服の色・持ち物の形の4つだけ。
7. 背景は透過ではなく、純粋なマゼンタ #FF00FF のベタ塗り。この色は被写体には絶対に使わない。
8. 1回につき1体(1枚)だけ描く。複数のポーズ・向き・キャラクターを並べない。
9. 画像の中に文字やロゴを入れない。
...
<main id="top">
  ...
  <p class="lead">1マス=32pxなので歩行キャラも32×32、<strong>頭身は2頭身</strong>。32ドットに入るのはシルエット・髪の色・服の色・持ち物の形の4つだけなので、人物の記述は群像譜の外見から<strong>この4つに絞り直して</strong>あります(「頬の刀傷」「片眼鏡」のような細部は、この大きさでは汚れにしかならないので落としました)。</p>
  <div class="callout">

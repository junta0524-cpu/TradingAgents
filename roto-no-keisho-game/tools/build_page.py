...
# 描き直し待ちの素材。人物は全部いったん白紙に戻したので、いまは無い。
REDO = set()
...
4. 人物は2頭身。**縦48ドットなら、髪のてっぺんから顎までが21〜24ドット** ―― 頭が全身の
   半分弱を占め、残りに胴と脚が入る。首は描かない。
...
<main id="top">
  ...
  <p class="lead">枠は<strong>32×48ドット</strong>(ゲームがそのまま使う寸法です)。人物は<strong>枠いっぱい</strong>に、
  足の裏を下端・髪のてっぺんを上端に合わせて描きます。
  32ドットに入るのはシルエット・髪の色・服の色・持ち物の形の4つだけなので、人物の記述は
  群像譜の外見から<strong>この4つに絞り直して</strong>あります
  (「頬の刀傷」「片眼鏡」のような細部は、この大きさでは汚れにしかならないので落としました)。</p>
  <div class="callout">
    <h3>いちばん外してはいけないのは頭身です</h3>
    <p><strong>縦48ドットのうち、髪のてっぺんから顎までが21〜24ドット。</strong>
    頭が全身の半分弱を占め、残りに胴と脚が入ります。ここが崩れると、色や輪郭がどれだけ正しくても
    「ドット絵風のイラスト」に見えてしまいます。各プロンプトの先頭に入れてあります。</p>
    <p>もうひとつ、<strong>届いた絵の中のドット数は毎回そろいません</strong>。
    比率(2:3)は守られるのですが、中身が32ドットだったり50ドットだったりします。
    32・64・96 のように<strong>32の倍数</strong>で返ってきたときがいちばん綺麗に縮むので、
    プロンプトに「等分したマスの中が単色であること」という確認の一文を足しました。</p>
  </div>

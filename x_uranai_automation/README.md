...
## 既知の注意点(2026年8月時点・変更が速い分野です)

X APIの料金体系、収益化プログラムの条件、LINE公式アカウントの料金プランはいずれも改定が頻繁です。**運用開始前に必ず各公式サイトで最新の条件を確認してください。**

- **文字数制限**：Xの280字は日本語などCJK文字を2カウントする「加重文字数」で判定されます。日本語のみの投稿は実質**140字が上限**です（`x_client.weighted_length` で計算・検証、`templates.MAX_CHARS = 120` で余裕を持たせて生成）。X Premium加入者は長文投稿(最大25,000字)が可能なため、Premium運用なら別途上限を調整してください。
- **画像アップロードAPI**：旧`POST /1.1/media/upload.json`は2025年3月に廃止されています。本ツールは現行の`POST /2/media/upload`(OAuth 1.0a User Context)を使用しますが、この領域はAPI仕様の変更が続いているため、本番投稿前に最新のX公式ドキュメントで確認してください。
- **X API料金**：2026年2月以降、新規契約は月額固定プランではなく従量課金がデフォルトです。無料枠だけでの本格運用は難しくなっています。
- **X Premium収益化プログラム**：目安として「X Premium加入」「認証済みフォロワー500人以上」「直近3ヶ月のインプレッション500万件以上」「Stripe連携」が条件とされています(2026年時点)。フォロワー500人は収益化の"入口"であり、収益が出る水準ではない点に注意。
- **GitHub Actionsのcron**：指定時刻ちょうどには実行されません。混雑状況により数分〜数十分(場合によってはそれ以上)遅延することがあります。またpublicリポジトリは60日間動きがないとscheduled workflowが自動的に無効化されます。
- **LINE公式アカウント無料プラン**：メッセージ配信は月200通まで(配信1回×友だち数でカウント)。友だちが増えるほど一斉配信できる頻度が下がるため、あいさつメッセージ(自動応答・通数外)を主軸にした設計が無料枠では現実的です。

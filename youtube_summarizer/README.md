...
#### スマホなど、同じWi-Fi上の別端末から使う

デフォルトではPC自身(`127.0.0.1`)からしかアクセスできません。同じWi-Fiに繋がったスマホ
などからも使えるようにするには、`HOST=0.0.0.0` を指定して起動します。

```bash
HOST=0.0.0.0 youtube-summarizer-web
```

起動したPCのLAN内IPアドレス(例: `192.168.1.10`。Macは「システム設定 > Wi-Fi」、Windowsは
`ipconfig`、Linuxは `hostname -I` などで確認できます)を使って、スマホのブラウザで
`http://<そのIPアドレス>:5000` を開いてください。PCとスマホが同じWi-Fiに接続されている
必要があり、PCのファイアウォールでポート5000が塞がれていないことも確認してください。

外出先など、自宅Wi-Fi以外からもアクセスできる本当の公開URLが必要な場合は、Render /
Railway / Fly.io といったホスティングサービスへのデプロイが必要です(このアプリ自体は
どんな環境でも動く通常のFlaskアプリなので、そのままデプロイできます)。
